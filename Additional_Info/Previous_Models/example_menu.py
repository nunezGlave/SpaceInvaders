...
def main_menu():
    ...
    run_button = pygame.Rect(SCREEN_WIDTH // 2 - 70, SCREEN_HEIGHT // 2 + 40, 140, 40)
    ...
    while True:
                    ...
                    try:
                        ...
                        for _ in range(num_instances):
                            if _ == 0 and attach_human:  # This is the first instance and human attached
                                subprocess.Popen(["python", "SpaceInvaders.py", "--attach-human"])
                            else:
                                subprocess.Popen(["python", "SpaceInvaders.py"])
                        pygame.quit()
                        sys.exit()
                    except ValueError:
                        pass
        ...
        pygame.draw.rect(screen, RED if attach_human else BLUE, attach_human_box)
        attach_human_surface = font.render('Attach Human?', True, (0, 0, 0))
        screen.blit(attach_human_surface, (attach_human_box.x + 30, attach_human_box.y - 5))
